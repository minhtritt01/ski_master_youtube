...
class Hud extends Component with ParentIsA<Viewport> {
  ...
  Future<void> onLoad() async {
    _snowman.position
        .setValues(parent.virtualSize.x - 35, parent.virtualSize.y - 20);
    _count.position.setValues(_snowman.position.x + 8, _snowman.position.y);
  ...
  void updateSnowmanCount(int count) {
    _count.text = 'x$count';
    _snowman.add(RotateEffect.by(pi / 8,
        EffectController(duration: 0.1, alternate: true, repeatCount: 2)));
    _count.add(ScaleEffect.by(
